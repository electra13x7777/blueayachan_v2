    ...
    */
    pub async fn execute_command(&self, name: String, client: Twitch_Client, msg_ctx: PrivmsgMessage) -> anyhow::Result<()>
    {
        ...
        if self.command_map.contains_key(&name)
        {
            ...
            let cmd_id: i32 = match query_command_id(&name)
            {
                Some(cmd_id) => cmd_id, // the correct command id
                None => 0, // invalid id, must be handled
            };
            if cmd_id != 0 && !io_flag.0
            {
                ...
                if let Some(cc) = query_channel_command(query_user_data(msg_ctx.channel_login.clone()), cmd_id.clone())
                {
                    ...
                    if cc.is_broadcaster_only && !io_flag.0
                    {
                    ...
                    if cc.is_mod_only && !io_flag.0
                    {
                        if !badges.contains(&"broadcaster".to_string()) && !badges.contains(&"moderator".to_string()) && !badges.contains(&"vip".to_string())
                        {
                            io_flag.0 = !io_flag.0;
                    ...
                    if cc.has_timeout
                    {
                        ...
                        let timeout_out: (bool, i32) = handle_command_timeout(query_user_data(msg_ctx.channel_login.clone()), bacuser, cmd_id, ndt_now, cc.timeout_dur.clone());
                ...
                else // VALIDATED
                {
                    ...
                    insert_channel_command(bac_channel, cmd_id.clone());
                }
            }
            ...
            if !io_flag.0
            {
                ...
                let res = String::from(callback(runtype, msg_ctx.clone()).await.unwrap());
                ...
                client.say(msg_ctx.channel_login.clone(), format!("{}", res)).await?;
            }
            else
            {
                match COLOR_FLAG
                ...
                client.say(msg_ctx.channel_login.clone(), format!("{}", io_flag.1)).await?;
            }
