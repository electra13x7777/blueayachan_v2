...
pub async fn query_srl(runtype: u8, msg_ctx: PrivmsgMessage) -> anyhow::Result<String> {
    match runtype {
        b'!' => {
            ...
            let req_str = format!(
                "https://www.speedrunslive.com/api/games?pageNumber={}&pageSize=1",
                page_num
            );
        ...
        b'?' => Ok(
            "This command queries a random speedgame using SRL\'s API. TOTAL_GAMES: 6576"
                .to_string(),
        ),
        b'#' => {
            let page_num: i32 = rand::thread_rng().gen_range(1..=6576);
            ...
            let tenshi_quote: &str = if pop == 0.0 {
                "Wow... no one plays this sh*t..."
            } else if pop >= 100.0 {
                "...insane popularity! CirnoGenius 🤝 SomaCruzFromAriaOfSorrow"
            } else if pop >= 20.0 {
                "Wow so popular! DataFace b"
            } else if pop < 20.0 {
                "Holy cow someone has played this game!"
            } else {
                "Wow... no one plays this sh*t..."
            };
