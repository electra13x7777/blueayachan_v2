...
pub async fn cfb(runtype: u8, _msg_ctx: PrivmsgMessage) -> anyhow::Result<String> {
    match runtype {
        b'!' => {
            let c_list: Vec<String> =
                readlines_to_vec("assets/c.txt").expect("Error reading file!");
            let f_list: Vec<String> =
                readlines_to_vec("assets/f.txt").expect("Error reading file!");
            let b_list: Vec<String> =
                readlines_to_vec("assets/b.txt").expect("Error reading file!");
            ...
            Ok(format!(
                "{} {} {}",
                c_list[index_c], f_list[index_f], b_list[index_b]
            ))
        }
        b'?' => Ok(
            "This command generates a string containing words that start with C, F, and B"
                .to_string(),
        ),
        _ => Ok(String::from("")),
    }
    ...
    match runtype {
        b'!' => {
            let id: i32 = rand::thread_rng()
                .gen_range(1..=get_kinohack_count())
                .try_into()
                .unwrap();
...
pub async fn strive(runtype: u8, msg_ctx: PrivmsgMessage) -> anyhow::Result<String> {
    match runtype {
        ...
        b'#' => {
            ...
            // check arg count
            if argv_s.len() != 1 {
                return Ok("Bad argument count! Please make sure your command follows this syntax: #strive <chatter>".to_string());
            }
            return Ok(format!(
                "{} accuses {} of being a Strive player!!!",
                msg_ctx.sender.name, argv_s[0]
            ));
        }
        _ => Ok(String::from("")),
    }
...
pub async fn shftnw(runtype: u8, msg_ctx: PrivmsgMessage) -> anyhow::Result<String> {
    match runtype {
        b'!' => {
            return Ok(format!(
                "{} loves Shadow Hearts: From the New World!",
                msg_ctx.sender.name
            ));
        }
        b'?' => {
...
generate_simple_command!(poll, "THERE'S STILL TIME TO VOTE IN THE POLL! https://take.quiz-maker.com/poll5038846x8D3f477D-153 CirnoGenius");
//generate_simple_command!(poll, "THERE'S STILL TIME TO VOTE IN THE POLL! http://bombch.us/DYOt CirnoGenius");
generate_simple_command!(
    repo,
    "You can find the github repository here: https://github.com/electra13x7777/blueayachan_v2"
);
generate_simple_command!(fsr, "🌻 ☀️ 🌧️");
//generate_simple_command!(weekly, "Last Week's Top 15: https://imgur.com/a/PYmokTp");
generate_simple_command!(
    ketchup,
    "https://clips.twitch.tv/ArtsyGracefulIguanaPhilosoraptor"
);
generate_simple_command!(
    fourthreefour,
    "https://twitter.com/LiquidSquid_/status/1215446601810042880?s=20"
);
