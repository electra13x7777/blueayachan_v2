...
pub fn handle_bac_user_in_db(user_nick_str: &str, twitch_id_str: &str)
{
    ...
    // CHECK
    let user_nick_lowercase = to_lowercase_cow(user_nick_str);
    let user_exists: bool = select(exists(blueayachanuser.filter(user_nick.eq(&user_nick_lowercase))))
    ...
    if !user_exists // FIRST TIME USING A COMMAND
    {
        ...
        {user_nick: user_nick_lowercase.as_ref(), num_commands: first_command, date_added: nt_now, twitch_id: twitch_id_str};
    ...
    else // ALREADY IN DATABASE
    {
        ...
        if bacuser.twitch_id == "Unregistered"
        {
            ...
            diesel::update(blueayachanuser
                .filter(user_nick.eq(&user_nick_lowercase)))
                .set(twitch_id.eq(twitch_id_str))
                .execute(&mut connection)
                .expect("Error updating user");
        }
        let _updated_row = diesel::update(blueayachanuser.filter(user_nick.eq(&user_nick_lowercase)))
...
pub fn query_user_data(user_nick_str: &str) -> BACUser
{
    ...
    let mut connection: PgConnection = establish_connection();
    let user_nick_lowercase = to_lowercase_cow(user_nick_str);
    let result = blueayachanuser.filter(user_nick.eq(user_nick_lowercase)).first::<BACUser>(&mut connection).expect("Oh no!");
    return result;
}
