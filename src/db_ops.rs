...
pub fn handle_bac_user_in_db(user_nick_str: &str, twitch_id_str: &str) {
    ...
    let user_exists: bool = select(exists(
        blueayachanuser.filter(user_nick.eq(&user_nick_lower)),
    ))
    ...
    // FIRST TIME USING A COMMAND
    {
        ...
        let new_bac_user = NewBACUser {
            user_nick: &user_nick_lower,
            num_commands: first_command,
            date_added: nt_now,
            twitch_id: twitch_id_str,
        };
    ...
    // ALREADY IN DATABASE
    {
        ...
        if bacuser.twitch_id == "Unregistered" {
            ...
            diesel::update(blueayachanuser.filter(user_nick.eq(&user_nick_lower)))
...
pub fn query_user_data(user_nick_str: &str) -> BACUser {
    ...
    let mut connection: PgConnection = establish_connection();
    let user_nick_lower: String = user_nick_str.to_lowercase();
    let result = blueayachanuser
        .filter(user_nick.eq(&user_nick_lower))
        .first::<BACUser>(&mut connection)
        .expect("Oh no!");
    return result;
}

// ONLY FOR COMMAND IMPLEMENTATION USE
pub fn query_user_data_by_tid(twitch_id_str: &str) -> BACUser {
    use crate::schema::blueayachanuser::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let result = blueayachanuser
        .filter(twitch_id.eq(twitch_id_str))
        .first::<BACUser>(&mut connection)
        .expect("Oh no!");
    return result;
}
...
pub fn insert_role(role_str: &str) {
...
pub fn insert_dbtweet(tweet_str: &str) {
    // parse out members
...
pub fn query_single_dbtweet(q_id: i32) -> String {
    ...
    use crate::schema::dreamboumtweets::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let result = dreamboumtweets
        .find(q_id)
        .first::<DBTweet>(&mut connection)
        .unwrap();
    return result.tweet;
}
...
pub fn insert_demon(name_str: &str, link_str: &str) {
    use crate::schema::nocturnedemons::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let new_demon = New_NDemon {
        demon_name: name_str,
        demon_img_link: link_str,
    };
    // insert
...
pub fn query_demon(q_id: i32) -> NDemon {
    ...
    use crate::schema::nocturnedemons::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let result = nocturnedemons
        .find(q_id)
        .first::<NDemon>(&mut connection)
        .unwrap();
    return result;
}
...
pub fn handle_user_last_demon(bacuser: &BACUser, demon: &NDemon, rarity: i32) {
    ...
    if !user_exists {
        ...
        let new_bac_user_demon = New_SavedNDemon {
            user_id: bacuser.id,
            saved_demon_id: sd_id,
            saved_demon_rarity: sd_r,
            last_demon_id: ld_id,
            last_demon_rarity: ld_r,
        };
        // insert
        ...
        diesel::update(bac_user_demons.filter(user_id.eq(&bacuser.id)))
            ...
            .execute(&mut connection)
            .expect("Error updating last demon ID");
        diesel::update(bac_user_demons.filter(user_id.eq(&bacuser.id)))
            .set(last_demon_rarity.eq(rarity))
            .execute(&mut connection)
            .expect("Error updating last demon RARITY");
    }
}
...
pub fn query_user_demon(bacuser: &BACUser) -> Option<SavedNDemon> {
    ...
    if !user_exists {
        ...
        let result = bac_user_demons
            .filter(user_id.eq(&bacuser.id))
            .first::<SavedNDemon>(&mut connection)
...
pub fn save_user_demon(bacuser: &BACUser) {
    ...
    if !user_exists {
        ...
        diesel::update(bac_user_demons.filter(user_id.eq(&bacuser.id)))
            ...
            .execute(&mut connection)
            .expect("Error updating saved demon ID");
        diesel::update(bac_user_demons.filter(user_id.eq(&bacuser.id)))
            .set(saved_demon_rarity.eq(&sud.last_demon_rarity))
            .execute(&mut connection)
            .expect("Error updating saved demon RARITY");
    }
}
...
pub fn query_pic_timeout(bacuser: &BACUser) -> Option<PicTimeout> {
    ...
    if !user_exists {
        ...
        let result = pictimeout
            .filter(user_id.eq(&bacuser.id))
            .first::<PicTimeout>(&mut connection)
            .expect("Error finding user");
        return Some(result);
    }
}
    ...
    if !user_exists {
        ...
        if diff >= timeout {
            diesel::update(pictimeout.filter(user_id.eq(&bacuser.id)))
                ...
                .execute(&mut connection)
                .expect("Error updating last pic timestamp");
            return (true, 0);
        }
        return (false, diff);
    }
...
// ADD BOT TO CHANNEL
pub fn insert_botchannel(user_nick_str: &str) -> bool {
    use crate::schema::botchannels::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let channel_exists: bool = select(exists(botchannels.filter(channel_name.eq(&user_nick_str))))
        .get_result(&mut connection)
        .unwrap();
    if !channel_exists {
        let bacuser: BACUser = query_user_data(user_nick_str);
        let ndt_now: NaiveDateTime = chrono::offset::Local::now().naive_local();
        let new_channel = NewBotChannel {
            channel_name: &bacuser.user_nick,
            channel_twitch_id: &bacuser.twitch_id,
            last_updated: ndt_now,
        };
...
pub fn query_command_id(command_name_str: &str) -> Option<i32> {
    ...
    let mut connection: PgConnection = establish_connection();
    let command_exists: bool = select(exists(blueayacommands.filter(name.eq(command_name_str))))
        .get_result(&mut connection)
        .unwrap();
    if !command_exists {
        return None;
    } else {
        let result = blueayacommands
            .filter(name.eq(command_name_str))
            .first::<BACommand>(&mut connection)
            .expect("Oh no!");
...
pub fn insert_channel_command(bacchannel: &BACUser, command_id_val: i32) {
    ...
    let new_cc = NewChannelCommands {
        channel_bac_id: bacchannel.id,
        command_id: command_id_val,
        is_active: true,
        is_broadcaster_only: false,
        is_mod_only: false,
        has_timeout: false,
        timeout_dur: 30,
        /*num_used: &0,*/ last_updated: nt_now,
    };
...
pub fn query_channel_command(bacchannel: &BACUser, command_id_val: i32) -> Option<ChannelCommands> {
    use crate::schema::channelcommands::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let channel_command_exists: bool = select(exists(
    ...
    if !channel_command_exists {
        ...
        let result = channelcommands
            .filter(
                channel_bac_id
                    .eq(&bacchannel.id)
                    .and(command_id.eq(&command_id_val)),
            )
            .first::<ChannelCommands>(&mut connection)
            .expect("Error finding Channel Command");
        return Some(result);
    }
}
...
pub fn set_channel_command_active(bacchannel: &BACUser, command_id_val: i32) -> (bool, String) {
    ...
    diesel::update(channelcommands.filter(channel_bac_id.eq(&ch_id).and(command_id.eq(&cmd_id))))
        ...
        .execute(&mut connection)
        .expect("Error updating channel command");
...
pub fn toggle_channel_command_active(bacchannel: &BACUser, command_id_val: i32) -> (bool, String) {
    ...
    if cc.is_active {
        diesel::update(
            channelcommands.filter(channel_bac_id.eq(&ch_id).and(command_id.eq(&cmd_id))),
        )
        .set((is_active.eq(false), last_updated.eq(&ndt_now)))
        .execute(&mut connection)
        .expect("Error updating channel command");
...
pub fn set_channel_command_broadcaster_only(
    bacchannel: &BACUser,
    command_id_val: i32,
) -> (bool, String) {
    use crate::schema::channelcommands::dsl::*;
    ...
    diesel::update(channelcommands.filter(channel_bac_id.eq(&ch_id).and(command_id.eq(&cmd_id))))
        .set((
            is_broadcaster_only.eq(true),
            is_mod_only.eq(false),
            last_updated.eq(&ndt_now),
        ))
        .execute(&mut connection)
        .expect("Error updating channel command");
...
pub fn set_channel_command_timeout_off(
    bacchannel: &BACUser,
    command_id_val: i32,
) -> (bool, String) {
    use crate::schema::channelcommands::dsl::*;
...
pub fn set_channel_command_timeout_duration(
    bacchannel: &BACUser,
    command_id_val: i32,
    timeout_dur_val: i32,
) -> (bool, String) {
...
pub fn query_command_timeout(
    ...
    let mut connection: PgConnection = establish_connection();
    let ct_exists: bool = select(exists(
        commandtimeout.filter(
            channel_bac_id.eq(bacchannel.id).and(
                user_bac_id
                    .eq(bacuser.id)
                    .and(command_id.eq(command_id_val)),
            ),
    ...
    if !ct_exists {
        ...
        let result = commandtimeout
            ...
            .first::<CommandTimeout>(&mut connection)
            .expect("Error finding ct");
        return Some(result);
    }
}

pub fn handle_command_timeout(
    bacchannel: &BACUser,
    bacuser: &BACUser,
    command_id_val: i32,
    ndt_now: NaiveDateTime,
    timeout: i32,
) -> (bool, i32) {
    use crate::schema::commandtimeout::dsl::*;
    let mut connection: PgConnection = establish_connection();
    let channel_ct_exists: bool = select(exists(
        commandtimeout.filter(
            channel_bac_id.eq(&bacchannel.id).and(
                user_bac_id
                    .eq(&bacuser.id)
    ...
    if !channel_ct_exists {
        ...
        let nct = NewCommandTimeout {
            channel_bac_id: bacchannel.id,
            user_bac_id: bacuser.id,
            command_id: command_id_val,
            last_command: ndt_now,
        };
        // insert
        ...
        let diff: i32 = ndt_now
            .signed_duration_since(ct.last_command)
            .num_seconds()
            .try_into()
            .unwrap();
        if diff >= timeout {
            ...
            .execute(&mut connection)
            .expect("Error updating last command timestamp");
            return (true, 0);
        }
        return (false, diff);
    }
...
insert_val_to_db!(unis, New_Uni, insert_uni);
...
query_string_simple!(unis, Uni, query_unis);
...
query_count_simple!(unis, get_uni_count);
